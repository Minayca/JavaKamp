...
public class ProductManager implements ProductService {
	...
	public DataResult<List<Product>> getAll(int pageNo, int pageSize) {
		...
		//Sayfa sayısı 0'dan başladığı için pageNo verdiğimiz değerin bir eksiği olmalıdır. 
		Pageable pageable = PageRequest.of(pageNo-1, pageSize);
		
		return new SuccessDataResult<List<Product>>
		(this.productDao.findAll(pageable).getContent());
		//Burada pageable findAll'un dönüş tipini okuyamıyor o yüzden getContent'ten yararlanıyoruz. 
	}

	@Override
	public DataResult<List<Product>> getAllSorted() {
		
		//Sort.by'ın ilk parametresi nasıl sıralanacağı, ikinci parametresi hangi alana göre sıralanacağıdır. 
